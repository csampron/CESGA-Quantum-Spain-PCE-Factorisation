...
def loss_func_estimator(
    ...
    # 🔴 CHECK AQUÍ (lo primero de todo)
    if np.isnan(x).any() or np.isinf(x).any():
        print(f"⚠️ ALERTA: x inválido → {x}")
        return np.inf
    ...
    if CUNQA == "Shots":
        ...
        """ print("dtype:", state_z.dtype)
        print("shape:", state_z.shape)
        print("nan:", np.isnan(state_z).any())
        print("inf:", np.isinf(state_z).any())
        print("norm:", np.sum(np.abs(state_z)**2)) """
    ...
    # Guiar a y b hacia la escala “heurística” del orden de sqrt(N), ayuda al optimizador a encontrar soluciones más razonables.
    prefactor_penalty = 10.0
    target_a_bits = np.sqrt(N_int)
    target_b_bits = np.sqrt(N_int)
    
    loss_heur_1=0
    #loss_heur_1 = prefactor_penalty*((a_int.bit_length() - target_a_bits)**2 + (b_int.bit_length() - target_b_bits)**2)


    # Penalización para evitar que (a-b)=0(mod N) y así evitar tener factores triviales
    loss_heur_2 = prefactor_delta*kron_delta((a_int - b_int) % N_int, 0)
    
    #print(f"loss_ab: {loss_ab}")

    loss = loss_pq + loss_ab + loss_heur_1 + loss_heur_2
