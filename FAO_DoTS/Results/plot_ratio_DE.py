...
for k_value in sorted(df["k"].unique()):
    ...
    # Filter by bit length
    if LIMIT_BIT_RANGE:
        df_k["bits"] = df_k["numero"].apply(int.bit_length)
        ...
        if df_k.empty:
            print(
                f"⚠️ No data for k={k_value} "
                f"in the range [{MIN_BITS}, {MAX_BITS}] bits."
            )
            continue
    ...
    if INCLUDE_TRIVIAL:
        plt.title(f"Success and trivial ratios per factorized number | k = {k_value} | DoTS Approach")
    else:
        plt.title(f"Success ratio per factorized number | k = {k_value} | DoTS Approach")
    ...
    if INCLUDE_TRIVIAL:
        filename = f"ratio_DoTS_success_trivial_k{k_value}_DE.png"
    else:
        filename = f"ratio_DoTS_success_k{k_value}_DE.png"
